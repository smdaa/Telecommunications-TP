...
%Etudes de chaines de transmission     8_PSK               
...
%mapping
symboles = pskmod(symboles_decimaux,M,0,'gray');
...
%décisions + demapping
z_dec = pskdemod(z_echan.',M,0,'gray');
demodbi=de2bi(z_dec.');
bits_decides=reshape(demodbi.',size(demodbi,1)*size(demodbi,2),1).';

%calcul du teb
TEB = sum(bits~=bits_decides)/length(bits);
assert(TEB == 0);
...
for k=1:length(EbN0) 
    ...
    %calcul du teb
    TEB_bruit(k) = sum(bits(2:end) ~= bits_decides(2:end))/(length(bits) -1) ; 
...
TEB_theo = ( 2 / log2(M) ) * qfunc(sqrt(2 * log2(M) * 10.^(EbN0db/10)) .* sin(pi/M));
figure;
